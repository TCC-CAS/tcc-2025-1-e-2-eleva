...
-- id BIGINT IDENTITY é a PK usada pelo JPA (@Id @GeneratedValue).
-- cpf permanece como chave única de negócio (usado em login e FKs legadas).
...
-- permiteReserva: 1 = estacionamento oferece reserva de vagas
-- qtdVagasReservaveis: quantas vagas são designadas para reserva (configuração do gestor)
...
-- clienteId referencia Cliente.id (PK IDENTITY, resolvida pelo JPA).
-- reservado: 1 = sessão criada via reserva de vaga.
...
-- Registra a entrada de um veículo via QR Code físico do operador.
-- Retorna sessaoId e horaEntrada via parâmetros OUTPUT.
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Idempotência: QR já processado → retorna dados da sessão existente
        IF EXISTS (
            SELECT 1 FROM SessaoEstacionamento WITH (UPDLOCK, HOLDLOCK)
            WHERE qrCode = @qrCode
        )
        BEGIN
            SELECT @sessaoId = id, @horaEntrada = horaEntrada
        ...
        IF @disponiveis IS NULL
        BEGIN
            ROLLBACK;
            THROW 50002, 'Estacionamento ou cliente não encontrado', 1;
        END
